...
/***************----- W5500 GPIO定义 -----***************/
#define SPI_W5500_CS_LOW() gpio_bit_reset(GPIOB, GPIO_PIN_12)
#define SPI_W5500_CS_HIGH() gpio_bit_set(GPIOB, GPIO_PIN_12)
extern uint8_t web_flag;
...
extern uint8_t Conmandn10[16]; //加载本机IP地址【0:3】，加载网关参数【4:7】加载端口号 【8:9】  子网掩码  【11:13】
extern uint8_t Conmandn20[8];  //MAC地址设置
extern uint8_t Conmandn30[8];  //信号参数修改码0-20/4-20mA参数设置
extern uint8_t Conmandn40[8];  //Lora配置，设备ID【0】，透传地址【1】；
extern uint8_t Conmandn50[8];  //485参数设置,波特率【0】，数据位【1】，停止位【2】，校验位【3】，控制流【4】，轮询时间【5】，规约【6】
extern uint8_t Conmandn60[16]; //Modbus配置1号机2号机，数据类型，高低字节顺序，功能码，设备地址，寄存器起始地址H，寄存器起始地址L，寄存器数量H，寄存器数量L
extern uint8_t Conmandn70[16]; //Modbus配置3号机4号机，数据类型，高低字节顺序，功能码，设备地址，寄存器起始地址H，寄存器起始地址L，寄存器数量H，寄存器数量L
extern uint8_t Conmandn80[16]; //Modbus配置5号机6号机，数据类型，高低字节顺序，功能码，设备地址，寄存器起始地址H，寄存器起始地址L，寄存器数量H，寄存器数量L
extern uint8_t Conmandn90[16]; //Modbus配置7号机8号机，数据类型，高低字节顺序，功能码，设备地址，寄存器起始地址H，寄存器起始地址L，寄存器数量H，寄存器数量L
...
void W5500_Initialization(void); //W5500初始化配置//装载网络参数		//STM32系统初始化函数(初始化STM32时钟及外设)
